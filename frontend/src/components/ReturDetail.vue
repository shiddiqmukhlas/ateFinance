  ...
  >
    <div class="bg-white w-full max-w-lg !py-6 !px-8 rounded-3xl shadow-lg relative">
      <div class="flex justify-between items-center mb-4">
        <h2 class="text-xl font-bold text-gray-800">Detail Retur</h2>
        <ion-icon 
      ...
      <div v-if="transaction" class="text-sm space-y-2">
        <p v-if="returStore.returDetail?.originalTransactionId">
          Original Transaction ID: {{ returStore.returDetail.originalTransactionId }}
        </p>
        <p><span class="font-semibold">ID Retur:</span> {{ transaction._id }}</p>
        <p><span class="font-semibold">Tanggal Retur:</span> {{ formatDate(transaction.tanggalTransaksi) }}</p>
        <p><span class="font-semibold">Grand Total Retur:</span> Rp {{ formatRupiah(transaction.grandTotal) }}</p>
        <p v-if="returStore.returDetail?.alasanRetur">
          <span class="font-semibold">Alasan Retur:</span> {{ returStore.returDetail.alasanRetur }}
        </p>

        <div>
          <h3 class="font-semibold mt-2 mb-2">Items Retur:</h3>
          <table class="w-full text-sm border-collapse">
            <thead>
              <tr class="bg-gray-100 text-gray-700">
                ...
                <th class="text-center !py-1 !px-2 border-b">Qty Retur</th>
...
const props = defineProps({
  ...
  returId: { type: String, required: true, default: '' },
});
...
// Fetch retur detail saat mounted
onMounted(async () => {
  if (props.returId) {
    try {
      await returStore.fetchReturById(props.returId);
      console.log('Data retur:', returStore.returDetail); // Pastikan ini memuat originalTransactionId dan alasanRetur
    } catch (error) {
      console.error('Error saat fetch retur:', error);
    }
  }
});
