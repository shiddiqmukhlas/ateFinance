<template>
    ...
    >
      <div class="bg-white w-full max-w-md !py-7 !px-10 rounded-3xl shadow-lg">
        <div class="flex justify-between">
          <h2 class="text-xl font-bold text-left mb-16">Edit Menu</h2>
          <ion-icon
  ...
  const handleSubmit = async () => {
    try {
      ...
      await menuStore.updateMenu(form.value.id, formData);
      emit('refreshTable');
      emit('close'); // Tutup dialog setelah sukses
    } catch (error) {
  ...
  const loadMenuData = async (id) => {
    try {
      if (!id) {
        console.error('ID is missing. Unable to load menu data.');
        return;
      }
      console.log('Loading data for ID:', id); // Debug
      const menu = await menuStore.getMenuById(id);
      if (!menu) {
        console.error('No menu found for the provided ID:', id);
        return;
      }
      form.value = {
        id: menu._id,
        ...
        harga: menu.harga,
          hargaFormatted: new Intl.NumberFormat('id-ID').format(menu.harga),

        status: menu.status,
        fotoMenu: null, // Foto hanya diisi jika diunggah ulang
      };
    } catch (error) {
  ...
  onMounted(() => {
    console.log('EditMenu mounted');
    if (menuStore.currentMenuId) {
        console.log('Editing menu ID:', menuStore.currentMenuId); // Debug
        loadMenuData(menuStore.currentMenuId);
    } else {
        console.error('No menu ID found to edit.');
    }
});
