  ...
  >
    <div class="bg-white w-full max-w-md !py-7 !px-10 rounded-3xl shadow-lg">
      <div class="flex justify-between">
        <h2 class="text-xl font-bold text-left mb-16">Tambah Pemasukan</h2>
        <ion-icon 
...
const form = ref({
  nominal: '', // Nominal asli (tanpa pemisah)
  nominalFormatted: '', // Nominal dengan format ribuan
  deskripsi: '',
  tanggal: new Date(), // Default ke format lokal
  kategori: '',
});
...
const formatNominal = () => {
  ...
  // Simpan nilai asli (tanpa pemisah) ke dalam form.nominal
  form.value.nominal = cleanValue;
};

const handleSubmit = async () => {
  try {
    // Ambil waktu lokal dari flatpickr
    ...
    await incomeStore.createIncome(form.value);
