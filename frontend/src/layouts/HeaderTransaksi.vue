<template>
    <nav class="bg-[#F7F7F8] h-20 !px-12 !py-4 fixed top-0 left-60 right-0">
      <div class="flex justify-between items-center">
        ...
        <div class="flex gap-0 shadow-sm rounded-md w-[23.75rem] mt-5">
            <button
            v-for="tab in tabs"
            :key="tab.name"
            :class="[
  ...
  const tabs = [
      { name: 'POS', route: '/transaksi/pos' },
      { name: 'Data Transaksi', route: '/transaksi/data-transaksi' },
      { name: 'Menu', route: '/transaksi/menu' },
  ];
